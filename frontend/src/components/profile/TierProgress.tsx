...
const TIERS = [
  ...
  { name: "Eco Starter", icon: "shield", minBottles: 10, maxBottles: 50 },
  { name: "Eco Hero", icon: "military_tech", minBottles: 50, maxBottles: 100 },
  { name: "Eco Champion", icon: "workspace_premium", minBottles: 100, maxBottles: 500 },
  { name: "Eco Legend", icon: "diamond", minBottles: 500, maxBottles: 9999 },
];
...
export default function TierProgress() {
  ...
  const progress = currentTier
    ? Math.min(100, ((totalScans - currentTier.minBottles) / (currentTier.maxBottles - currentTier.minBottles)) * 100)
    : 0;
  ...
  return (
    <div className="bg-surface-container-lowest rounded-2xl p-6 shadow-[0px_24px_48px_rgba(17,28,45,0.06)]">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {TIERS.map((tier, i) => (
          <div key={tier.name} className={`flex-1 min-w-[80px] p-3 rounded-xl text-center transition-all ${i === currentIdx ? "bg-primary text-on-primary shadow-md shadow-primary/20"
              : i < currentIdx ? "bg-secondary-container/50 text-on-secondary-container"
                : "bg-surface-container text-outline opacity-60"
            }`}>
            <span className="material-symbols-outlined text-xl mb-1 block" style={{ fontVariationSettings: i <= currentIdx ? '"FILL" 1' : '"FILL" 0' }}>{tier.icon}</span>
